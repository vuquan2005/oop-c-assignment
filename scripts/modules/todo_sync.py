...
def parse_header_comment(file_path, file_name):
    ...
    info = {"file": "", "brief": "", "note": [], "author": "", "date": "", "status": ""}
...
def sync_todo() -> bool:
    ...
    for folder_name in subdirs:
        ...
        readme_items = [
            format_item(info, relative_path) for info, relative_path, _ in folder_items
        ]
        todo_items = [
            format_item(info, relative_path) for info, _, relative_path in folder_items
        ]
        ...
        if os.path.exists(readme_path):
            ...
            updated_readme = replace_auto_generated_block(
                readme_content, folder_new_content
            )
            ...
            updated_readme = replace_auto_generated_block(
                new_readme_template, folder_new_content
            )
            with open(readme_path, "w", encoding="utf-8") as r:
                r.write(updated_readme)
        ...
        section_md = (
            f"### 📅 Bài tập [{display_name}]({folder_link})\n\n"
            + "\n\n".join(todo_items)
        )
